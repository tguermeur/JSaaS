...
saveButton.addEventListener('click', async () => {
  ...
  try {
    ...
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:486',message:'domPhotoUrl from response',data:{domPhotoUrl:domPhotoUrl ? domPhotoUrl.substring(0,150) : '',hasPhoto:!!domPhotoUrl,photoUrlLength:domPhotoUrl?.length || 0,isValid:isValidLinkedInPhotoUrl(domPhotoUrl)},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
      // #endregion
      ...
      if (response.fromCache) {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:492',message:'Using cached data',data:{photoUrl:profileData.photoUrl ? profileData.photoUrl.substring(0,150) : '',hasPhoto:!!profileData.photoUrl},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
        // #endregion
        ...
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:508',message:'Before setting photoUrl from DOM',data:{domPhotoUrl:domPhotoUrl ? domPhotoUrl.substring(0,150) : '',domPhotoUrlValid:isValidLinkedInPhotoUrl(domPhotoUrl),extractedPhotoUrl:extracted.photoUrl ? extracted.photoUrl.substring(0,150) : ''},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
        // #endregion
        ...
        if (domPhotoUrl && isValidLinkedInPhotoUrl(domPhotoUrl)) {
          ...
          // #region agent log
          fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:513',message:'Photo set from DOM',data:{photoUrl:profileData.photoUrl.substring(0,150)},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
          // #endregion
        } else {
          // Si pas de photo DOM valide, vider le champ (éviter les URLs corrompues de l'IA)
          profileData.photoUrl = '';
          console.log('Pas de photo de profil valide trouvée', { domPhotoUrl, isValid: isValidLinkedInPhotoUrl(domPhotoUrl) });
          // #region agent log
          fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:517',message:'Photo NOT set - invalid or missing',data:{domPhotoUrl:domPhotoUrl ? domPhotoUrl.substring(0,150) : '',isValid:isValidLinkedInPhotoUrl(domPhotoUrl),reason:!domPhotoUrl ? 'empty' : 'invalid'},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
          // #endregion
        }
      ...
      // #region agent log
      fetch('http://127.0.0.1:7243/ingest/510b90a4-d51b-412b-a016-9c30453a7b93',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'popup.js:544',message:'Before saving to Firebase',data:{photoUrl:profileData.photoUrl ? profileData.photoUrl.substring(0,150) : '',hasPhoto:!!profileData.photoUrl,photoUrlLength:profileData.photoUrl?.length || 0,name:profileData.name},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'G'})}).catch(()=>{});
      // #endregion
